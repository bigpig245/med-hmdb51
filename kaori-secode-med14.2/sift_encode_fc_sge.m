...
%% kf_dir_name: name of keyframe folder, e.g. keyframe-60 for segment length of 60s   
	...
	output_dir = sprintf('/home/ntrang/%s/feature/%s/%s', proj_name, exp_ann, feature_ext);
    if ~exist(output_dir, 'file'),
		mkdir(output_dir);
		change_perm(output_dir);
    end
    
    codebook_file = sprintf('/home/ntrang/%s/feature/bow.codebook.devel/%s.%s.bg.sift/data/codebook.gmm.%d.%d.mat', ...
    ...
    codebook = codebook_.codebook;
 	...
 	low_proj = [];
	if dimred < default_dim,
		lowproj_file = sprintf('/home/ntrang/%s/feature/bow.codebook.devel/%s.%s.bg.sift/data/lowproj.%d.%d.mat', ...
    ...
    kf_dir = sprintf('/home/ntrang/%s/keyframes', proj_name);
