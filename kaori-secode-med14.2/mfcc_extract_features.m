...
%   Detailed explanation goes here
	...
	% addpath('/home/ntrang/tools/mfcc');
	tmpdir = '/home/ntrang/tmp/mfcc';
	
	ffmpeg_bin = '/home/ntrang/usr.local/ffmpeg-1.2.1/release/bin/ffmpeg';
	...
	%%% /home/ntrang/usr.local/ffmpeg-1.2.1/release/bin/ffmpeg -i /home/ntrang/dataset/MED10/HVC1035.mp4 2>&1 | sed -n "s/.*, \(.*\) fps.*/\1/p"
	%%% example 2, with one output, using sed '0,/pattern/s/pattern/replacement/' filename
	%%% /home/ntrang/usr.local/ffmpeg-1.2.1/release/bin/ffmpeg -i /home/ntrang/dataset/MED10/HVC1035.mp4 2>&1 | sed -n "0,/.*, \(.*\) fps.*/s/.*, \(.*\) fps.*/\1/p"
	...
	%%cmd = sprintf('/home/ntrang/usr.local/ffmpeg-1.2.1/release/bin/ffmpeg -i %s -vn -ac 1 -ar 44100 -ab 320k -f wav %s', filepath, tmp_audio_file);
...
function log (msg)
	fh = fopen('/home/ntrang/tools/kaori-secode-med11/log/mfcc_extract_features.log', 'a+');
	fprintf(fh, [msg, '\n']);
	fclose(fh);
end
