...
%   Detailed explanation goes here
    ...
    tmpdir = '/home/ntrang/projects/tools/mfcc';
	...
	if ~exist('start_frame', 'var') && ~exist('end_frame', 'var'),
		cmd = sprintf('ffmpeg -i %s -v quiet -y -vn -ac %d -ar 44100 -ab 320k -f wav %s', filepath, ac_param, tmp_audio_file);
	else
		%%% get fps, note: escape \ by \\
		cmd = sprintf('ffmpeg -i %s 2>&1 | sed -n "0,/.*, \\(.*\\) fps.*/s/.*, \\(.*\\) fps.*/\\1/p"', filepath);
		[~, fps] = system(cmd);
		...
		cmd = sprintf('ffmpeg -i %s -v quiet -y -ss %s -to %s -vn -ac %d -ar 44100 -ab 320k -f wav %s', filepath, ss_time, to_time, ac_param, tmp_audio_file);
    end
...
function log (msg)
	fh = fopen('/home/ntrang/projects/logs/mfcc_extract_features.log', 'a+');
	fprintf(fh, [msg, '\n']);
	fclose(fh);
end
