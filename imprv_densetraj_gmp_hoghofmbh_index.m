function imprv_densetraj_gmp_hoghofmbh_index(index, kernel)
...
% Detailed explanation goes here
	...
	for i = index:length(metadata.videos),
		...
		if ~exist(hoghof_code_file, 'file'),
			fprintf(' Does not exist hoghof encoding file for %s\n', i, video_name);
			continue;
		end
		
		if ~exist(mbh_code_file, 'file'),
			fprintf(' Does not exist mbh encoding file for %s\n', i, video_name);
			continue;
		end
